...
- 한 턴에 도구 모델 1회(빠름·소형) → (필요 시) 도구 실행 → 대화 모델 1회.

### 2.1 조건부 활성화 (중복 로딩 방지) — 핵심 원칙

**두 번째(도구) 모델은 대화 모델이 툴콜을 못 할 때만 올린다.** 이미 툴콜이 되는 모델을 쓰면 그 모델
하나로 처리하고 라우터/2번째 모델은 띄우지 않는다.

```
대화 모델 선택
   │
   ├─ supportsFunctionCalling == true (Gemma·Qwen3)
   │      └─► 단일 모델 경로(현행): 그 모델이 대화+툴콜. 2번째 모델 ❌
   │
   └─ supportsFunctionCalling == false (Qwen2.5·DeepSeek·Llama…)
          └─► 라우터 경로: 작은 Gemma를 "도구 모델"로 사용
                - 지연 로딩: 첫 도구가 실제로 필요할 때 로드(단순 수다만 하면 안 올림)
                - 사용 후 일정 시간 미사용 시 언로드(메모리 회수) 고려
```

- 판별은 기존 `VoiceAssistantTask.supportsFunctionCalling(model)`을 재사용.
- 효과: 툴콜 지원 모델은 메모리/지연 추가 0, 미지원 모델만 필요 시 보조 모델로 능력 보강.
- 도구 모델 후보: 변환 없이 바로 쓰는 **작은 Gemma**(예: Gemma 3 1B). 추후 FunctionGemma
  `.litertlm`이 준비되면 도구 모델만 교체(더 가볍고 정확).
