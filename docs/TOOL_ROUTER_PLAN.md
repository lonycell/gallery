...
### 2.2 도구 모델은 교체 가능한 "설정"으로 (UI 포함)

도구 모델은 코드에 하드코딩하지 않고 **사용자가 고르는 설정**으로 둔다.

- **저장**: 전역 설정 `toolModelName: String`(빈 값 = "자동"). `CharacterRepository`에 prefs로 영속화
  (defaultVoiceId와 동일 패턴).
- **후보 목록**: 다운로드된 모델 중 **툴콜을 지원하는 것만**(`supportsFunctionCalling == true`).
  툴콜 못 하는 모델은 도구 모델이 될 수 없으므로 목록에서 제외.
- **기본값 "자동"**: 후보 중 가장 작은(=가벼운) 모델을 자동 선택. 미설치면 도구 비활성(일반 채팅).
- **UI**: 음성 설정 화면에 "도구 담당 모델" 섹션 추가 — `[자동 | <툴콜 모델들…>]` 칩/드롭다운.
  바꾸면 즉시 다음 턴부터 적용. 대화 모델이 이미 툴콜을 지원하면(2.1) 이 설정은 의미가 없으므로
  비활성/숨김 처리(또는 "대화 모델이 직접 처리 중" 표시).
- **확장성**: 나중에 FunctionGemma `.litertlm`을 받으면 후보 목록에 자동으로 떠서, **선택만 바꾸면**
  도구 모델이 교체된다(코드 변경 없음).

라우터는 이 설정을 읽어 도구 모델을 고르고, "자동"이면 위 규칙으로 결정한다.
...
### 5.0 호출 처리 방식: 보조(도구) 모델은 항상 "expose-only"로 통일 (핵심 결정)

런타임의 함수호출은 두 방식이 있다:

- **자동 실행형(auto-execute)**: 런타임에 도구 구현(`@Tool`)을 등록 → 모델이 호출을 내면 **런타임이
  직접 실행**하고 최종 답까지 만든다(현재 Gemma 단일 모델 경로).
- **호출만 노출(expose-only)**: 런타임/모델은 호출(이름+인자)만 출력하고 멈춘다 → **앱이 실행**하고
  결과를 모델에 되먹인다(OpenAI tool_calls, FunctionGemma 방식).

**결정: 경로 B의 보조 도구 모델은 항상 expose-only로 다룬다.** 즉 litert 자동실행(ToolSet)을 쓰지 않고,
"도구 목록을 줄 테니 어떤 걸 호출할지 **구조화된 텍스트로 출력**" 시킨 뒤 **앱이 파싱·실행**한다.

- 이유: 런타임의 자동실행 여부와 무관하게 동일 코드로 동작 → FunctionGemma든 일반 Gemma든 동일.
  그리고 실행 결과를 **대화 모델(페르소나)** 이 받아 자연스럽게 답하게 할 수 있다.
- `ToolRouter.route() → List<ToolCall>` 인터페이스가 이 추상화다(구현이 무엇이든 바깥은 동일).
- 두 경로의 통일: **A**(대화 모델 툴콜 지원)는 런타임 자동실행 그대로, **B**는 expose-only 수동 루프 —
  둘 다 `ToolRouter` 뒤에 숨겨 바깥 흐름은 하나.

### 5.1 엔진/프롬프트/파싱

- **두 개의 Engine**: 대화 모델 Engine(기존) + 도구 모델 Engine(신규, 소형). 둘 다 메모리에
  상주(예: Gemma 1B/270M은 양자화 시 수백 MB~1GB라 현대 폰에서 동시 로딩 가능). 경로 B에서만 로딩.
